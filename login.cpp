...
bool Dialog::login()
{
   ...
   // char buf1[4]={1,2,3};char buf2[4]={2,3,4};
    ...
    if(status=='1')
    {
        ...
        ui->message->setText("don't contain this name");
        return 0;
    }
    if(status=='2')
    {
        cout<<"name password not match;"<<endl;
        ui->message->setText("name password not match");
        return 0;
    }
    if(status=='0'){
        char ff[6]="money";
...
void Dialog::on_pushButton_2_clicked()
{
    cout<<"triger"<<endl;
    if(ui->le_name->text()=="")
        ui->message->setText("input your name");
    else{
        if(ui->le_password->text()=="")
            ui->message->setText("input password");
         else{
            std::string name=(ui->le_name->text()).toStdString();
            std::string password=(ui->le_password->text()).toStdString();
            list<pair<string,string>> l;
            //m.insert(pair<string,string>("type",stype));
            l.push_back(pair<string,string>("type",to_string(REGISTER)));
            l.push_back(pair<string,string>("name",name));
            l.push_back(pair<string,string>("password",password));
            char send_buf[1000],recv_buf[1000];
            bzero(send_buf,sizeof(send_buf));bzero(recv_buf,sizeof(recv_buf));
            ...
            cout<<status<<endl;
            if(status=='1')
                ui->message->setText("register success");
            else
                ui->message->setText("register error");
        }
    }
